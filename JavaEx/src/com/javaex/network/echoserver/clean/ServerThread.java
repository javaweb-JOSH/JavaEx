package com.javaex.network.echoserver.clean;
...
public class ServerThread extends Thread {
	...
	public void run() {
			try {
		//			클라이언트 정보 확인
			InetSocketAddress socketAddress =(InetSocketAddress) socket.getRemoteSocketAddress();	//	원격지 소켓의 주소확인
			...
			System.out.println("	클라이언트:" + socketAddress.getAddress() + ":" + socketAddress.getPort());
				...
				while(true) {
					...
					if (message == null) {	//	더 이상 읽을 메시지 없음
