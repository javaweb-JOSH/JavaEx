...
//			except.executeChecked();
			...
			//	혹시 남아있을 수 있는 예외를 위해 마지막은 Exception catch
			e.printStackTrace();
		} finally {
	...
	private static void arrayExceptionEx() {
		...
		try {
			System.out.println(intArray[3]);	//	ArrayIndexOutOfBoundsException 배열범위 밖
	...
	private static void arithExceptionEx() {
		...
		try {
			...
			//	구체적인 예외 처리 후 남은 예외들 (=ExceptionEX 가장 마지막에)
